...
- (nullable NSString *)cachedFileNameForKey:(nullable NSString *)key {
    ...
    NSURLComponents *commpoents = [NSURLComponents componentsWithString:key];
    if (commpoents && commpoents.path) {
        pathExtension = [commpoents.path pathExtension];
    } else {
        pathExtension = [key pathExtension];
    }
    NSString *filename = [NSString stringWithFormat:@"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%@",
                          r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10],
                          r[11], r[12], r[13], r[14], r[15], [pathExtension isEqualToString:@""] ? @"" : [NSString stringWithFormat:@".%@", pathExtension]];
...
- (nullable UIImage *)imageFromDiskCacheForKey:(nullable NSString *)key {
    ...
    if (diskImage && self.config.shouldCacheImagesInMemory && key) {
    ...
    if (object) {
        ...
        if ([object isKindOfClass:[UIImage class]]) {
            ...
            if ([NSData sd_imageFormatForImageData:diskData] == SDImageFormatGIF) {
                image = [UIImage sd_animatedGIFWithData:diskData];
            }
        }
